...
const Register = () => {
    ...
    const LoginClicked = async () => {
        ...
        try {
            setLoading(true);
            const response = await axios.post('https://kuricmt-backend.onrender.com/user/verify-email', {
                email,
                name
            });
    ...
    const VerifyOTP = async (e) => {
        ...
        try {
            ...
            const reponse = await axios.post('https://kuricmt-backend.onrender.com/user/verify-otp', {
                email,
                otp: notp
            });
                ...
                <div className="w-full lg:w-2/3 lg:mx-auto h-screen items-center lg:flex  px-6 bg-white">
                    ...
                    <div className="w-full lg:w-1/2">
                        <div className='lg:w-2/3 box-border'>
                            ...
                            <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder='Email ID' className='outline-none bg-gray-100 px-4 py-1 border-l-2 w-full border-blue-500 my-2 block' />
                        ...
                        <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
                            {
                                ...
                                <div className='flex items-center bg-red-200 px-4 py-3'>
                                    <div className='w-2/12'>
                                        <span className='text-red-600 text-4xl'><IoIosWarning /></span>
                                    </div>
                                    <div className='w-10/12'>
                                        <span className='font-semibold text-sm'>{errorText}</span>
                                    </div>
                            ...
                            <div className='flex justify-between items-center my-2'>
                                {
                                    timeLeft > 0 ?
                                        ...
                                        <h1 className='text-2xl font-bold mb-4 text-red-700'>Expired</h1>
                                }
                                {
                                    timeLeft > 0 ? <span className='text-sm'>Will Expire In {formatTime(timeLeft)}</span> : <div>
                                        <button className='inline-block px-4 py-1 text-sm font-semibold text-center text-white uppercase transition-all duration-300 bg-gradient-to-r from-blue-500 to-blue-800 rounded-full shadow-md hover:from-blue-600 hover:to-blue-900 hover:shadow-lg focus:outline-none focus:ring-4 focus:ring-blue-300 active:shadow-inner' onClick={()=>LoginClicked()}>Resend OTP</button>
                                    </div>
                                }
                    ...
                    <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-500 bg-opacity-50">
                        <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
                            <div className='flex items-center justify-center py-4 flex-col'>
                                ...
                                <p className='font-semibold text-gray-700 pt-3'>Redirecting in 3s</p>
                            </div>
                        </div>
                    </div>
                </div>
